...
%"maxfreq" is the maximum frequency to be sampled (Default=Nyquist)
...
% If you want to "hardwire" minfreq & maxfreq & samplingrate &
% freqsamplingrate do it here

% calculate the sampled time and frequency values from the two sampling rates
t = (0:length(timeseries)-1)*samplingrate;%生成的时间序列
spe_nelements =ceil((maxfreq-minfreq+1)/freqsamplingrate);
% f为生成的频率序列,通过samplingrate输入采样间隔,然后确定Nyquist频率,生成的f序列即为0-Nyquist之间的序列
